      ...
      <div class="card">
        ...
        <div class="card-body no-padding">
          <?php
                ...
                $cell_add = array('data' => '<a href="'.site_url().'/users/add">Add New Data</a>', 'class' => 'highlight', 'colspan' => 2);

                $this->table->set_heading('UserID','Username','Status','Doctor ID','Worker ID','Created Date','Updated Date','Action');
                foreach($query as $row){
                    $edit = "<a href='".site_url()."/users/edit/".$row->id_user."' title='".$row->username."'>Edit</a>
                    <br>
                    <a href='".site_url()."/users/delete/".$row->id_user."' title='".$row->username."' onclick='return confirmDelete();'>Delete</a>"; 
